...
| Title           | WCCChallenge 2025 Week 41 - Creature
| 📅 Started      | 2025-10-12      
| 📅 Completed    | 2025-10-12        
| 🕒 Taken        | 6hrs                     
| 🤯 Concept      | "Mildly Upsetting" was the aim... How did I go?
| 🔎 Focus        | Blending multiple shapekeys with simplex noise

Made for Sableraph's weekly creative coding challenges, reviewed weekly on https://www.twitch.tv/sableraph
See other submissions here: https://openprocessing.org/curation/78544
Join The Birb's Nest Discord community! https://discord.gg/g5J6Ajx9Am

Tried not to use AI nearly as much this time and I definitely benefited from taking on the cognitive load 😅

Each shapekey's influence on the mouths is driven by a different simplex noise value: Smile, Sneer, Sad and Pucker
Used easings functions to make it more likely the shapekeys would be either on or off but with nice transitions between


RESOURCES
- Wet Mouth Sounds 1: https://pixabay.com/sound-effects/marshmallow-mouth-sounds-asmr-295418/
- Wet Mouth Sounds 2: https://pixabay.com/sound-effects/wet-mouth-sounds-asmr-2-295416/
- Horror Ambience: https://pixabay.com/music/mystery-creepy-horror-ambience-381940/
- Bump Map (upped resolution in photoshop): https://www.filterforge.com/filters/1610-bump.html
...
const mouthFolder = gui.addFolder('mouthBiz');
mouthFolder.add(mouthParams, 'manualMode');
mouthFolder.add(mouthParams, 'smileInfluence', 0, 1);
mouthFolder.add(mouthParams, 'sneerInfluence', 0, 1);
mouthFolder.add(mouthParams, 'sadInfluence', 0, 1);
mouthFolder.add(mouthParams, 'puckerInfluence', 0, 1);
...
function init() {
    ...
    // 6.2 Set up locations of biz
    locations = fibonacciSphere(params.n);
    for(let i = 0; i < locations.length; i++){
        locationsTypes[i] = {location: locations[i], type: Math.random() < params.mouthProbability ? "MOUTH" : "EYE"};
    }
...
function animate() {
    ...
    if(!params.manualEyeControl){
        for(let currentEye of eyes){
            currentEye.updateEyelid(5*t);
            currentEye.updateEye(7.5*t);
        }
    }

    // animate the mouths
    if(!mouthParams.manualMode){
        for(let currentMouth of mouths){
            currentMouth.update(5*t);
        }
    } else {
        for(let currentMouth of mouths){
            currentMouth.updateMorph(0, mouthParams.smileInfluence);
            currentMouth.updateMorph(1, mouthParams.sneerInfluence);
            currentMouth.updateMorph(2, mouthParams.sadInfluence);
            currentMouth.updateMorph(3, mouthParams.puckerInfluence);
        }
    }
...
function fibonacciSphere(numPoints) {
    ...
    for (let i = 0; i < numPoints; i++) {
        ...
        const theta = goldenAngle * i + (Math.random() - 0.5)*Math.PI/24; // slight perturbation;
