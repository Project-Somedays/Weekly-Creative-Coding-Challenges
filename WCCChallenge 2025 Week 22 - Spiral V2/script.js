...
function archimedesSpiral(a, t, h){
  let x = a * t * Math.sin(t);
  let z = a * t * Math.cos(t);
  let y = p5map(t, -2*TWO_PI, 2*TWO_PI, -h, h);
...
function animate() {
    ...
    frameCount ++
    let t = TWO_PI * sin(Math.PI * frameCount / cycleFrames);
    // Rotate the cube for demonstration
    for(let i = 0; i < cubeCount; i++){
        let tOff = p5map(i/cubeCount, 0, 1, -TWO_PI, Math.PI);
        let p = archimedesSpiral(0.1, tOff + t, 2);
        cubes[i].position.set(p.x, p.y, p.z);
...
function p5map(val, minA, maxA, minB, maxB){
    return (maxB - minB) * (val / (maxA - minA));
}
