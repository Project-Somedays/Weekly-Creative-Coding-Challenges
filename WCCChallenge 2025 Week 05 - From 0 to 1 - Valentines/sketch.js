...
function preload(){
  ...
  heart = loadModel("Heart.obj", true, () => console.log("Successfully loaded heart model! 🙂"), () => console.log("Uh oh. Error loading heart model 😢"));
  }
...
function setup() {
  ...
  drawOnGraphic(canvasZero, 'LOVE');
  drawOnGraphic(canvasOne, 'YOU!');
  rectMode(CENTER);
  ortho();
      ...
      for(let z = -width/2; z < width/2; z += step){
        ...
        if(brightness(cnvA.get(graphicsX, graphicsY)) === 100 && 
          ...
          validPoints.push({
            ...
            m: heart,//random() < 0.5 ? zeroModel : oneModel, 
