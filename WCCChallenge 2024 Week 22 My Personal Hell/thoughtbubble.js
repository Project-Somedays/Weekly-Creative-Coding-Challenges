class ThoughtBubble{
    constructor(x, y, majorAxis, minorAxis, numOfCircles){
      ...
      for(let i = 0; i < numOfCircles; i++){
        ...
        this.circles.push({
          ...
          'offset' : off,
          's' : map(noise(off + frameCount/bubbleRate), 0, 1, (width,height)/15, max(width, height)/4)
        })
      }
    }
    ...
    update(){
        for(let c of this.circles){
            c.s = map(noise(c.offset + frameCount/bubbleRate), 0, 1, (width,height)/10, max(width, height)/5);
        }
    }
